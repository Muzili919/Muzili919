...
| 生图失败/出废图，照样发出去了 | 图片质量闸：尺寸、体积、格式、纯色检测；不达标就不发（`image.required`） |
...
> 这里原本是"配图失败降级为纯文字继续发"，理由是图只是锦上添花。**该设计已作废**：现在的规则是每条内容都必须带图、且图一律由 GPT 生成——纯文字帖在推荐和完读上都吃亏，拿不到图宁可今天不发。由 `image.required` 控制，默认 `true`。
...
- **CDP 生图这条链路天然脆**：依赖 ChatGPT 前端 DOM，官方改版就会失效。而现在图是必需项（`image.required`），所以这条链路一挂，当天就发不出去——**它从"锦上添花"变成了关键路径**。2026-07 就撞过一次：ChatGPT 换了图片存放地址，`_SELECTORS['generated_image']` 三个候选全不匹配，图明明生成了却认不出来，白等 240 秒报"超时"。想彻底稳下来，换成图片 API（即梦/通义/DALL-E）是更省心的路，`images/` 下加一个新实现、在 `pipeline.py` 里换掉即可。
